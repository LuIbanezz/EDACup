...
void Keeper :: positionGK()
{
    ...
    float angleTo1st = angleBetweenVectors(ballTo1st,{TEAM_SIGN * 1, 0});
    ...
    if (shot)
    {
        ...
        Vector3 diveVector = Vector3Scale(Vector3CrossProduct(controller->ball.speed,{0,0,-1}),TEAM_SIGN);
        
        Vector2 diveVector2D={diveVector.x,diveVector.y};
        float diveAngle = angleBetweenVectors(diveVector2D,{TEAM_SIGN * 1, 0}) * PI/180;
    ...
    else
    {
        ...
        if (forwardStep <= 0)
            forwardStep = 0;
        float x = TEAM_SIGN * (4.5-forwardStep);
        float y = ball2D.y + slope * (x - ball2D.x);
        moveRobot({x, y}, MAX_SPEED);
    }
}
